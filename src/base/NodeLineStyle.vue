<template>
  <div class="note-lines-style-container">
    <a-row type="flex" align="middle" :gutter="[16, 16]">
      <a-col :span="12">
        <a-row type="flex" align="middle" :gutter="[16, 16]">
          <a-col :span="7">结构</a-col>
          <a-col :span="17">
            <a-dropdown-button>
              <div class="shape-list" slot="overlay">
                <svg
                  width="60"
                  height="26"
                  v-for="(item, index) in structPath"
                  :key="index"
                  class="shape-one"
                  style="margin-top: 10px"
                  @click="handleSelectStruct(item)"
                >
                  <path :d="item.path1" fill="none" stroke="black" stroke-width="2"></path>
                  <path :d="item.path2" fill="none" stroke="black" stroke-width="2"></path>
                </svg>
              </div>
              <svg width="60" height="26">
                <path :d="selectedStruct.path1" fill="none" stroke="black" stroke-width="2"></path>
                <path :d="selectedStruct.path2" fill="none" stroke="black" stroke-width="2"></path>
              </svg>
...
import { STRUCT_PATH } from '@/config'
export default {
  name: 'NodeLineStyle',
  filters: {},
  components: {
  ...
  methods: {
    ...
    handleSelectStruct(structPath) {
      this.selectedStruct = structPath
    },
    // 设置颜色
    handleColorChange() {}
  }
