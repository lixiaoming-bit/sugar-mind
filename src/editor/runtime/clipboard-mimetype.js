...
function MimeType() {
  ...
  this.registMimeTypeProtocol = function (type, sign) {
    if (sign && SIGN[sign]) {
      throw new Error('sing has registered!')
    }
...
MimeType.prototype.isPureText = function (text) {
  return !~text.indexOf(this.getSplitOr())
}
...
MimeType.prototype.getPureText = function (text) {
  ...
  return text.split(this.getSplitOr())[1]
}
...
MimeType.prototype.whichMimeType = function (text) {
  ...
  return this.getMimeType(text.split(this.getSplitOr())[0])
}
